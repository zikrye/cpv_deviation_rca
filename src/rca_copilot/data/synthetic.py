...
The data is curated to tell one coherent investigation story:
  ...
  CAPA, a process signal and an assay investigation) cluster around the
  *Material* and *Method* fishbone categories. The Measurement records, by
  contrast, indicate the titer assay itself was in control — so it should rank
  lower. The point of the demo is to *prioritize candidate causes by evidence*,
  not to confirm any of them.
"""
...
#: Lower spec limit on harvest titer (g/L). Below this is out-of-spec (OOS);
#: between LSL and the lower control limit is out-of-trend (OOT).
LSL_TITER = 4.0
...
def generate_batches(seed: int = SEED) -> pd.DataFrame:
    """Monthly campaign of harvest titer results; XX0007/XX0008 run low."""
    rng = np.random.default_rng(seed)
    dates = pd.date_range("2025-01-15", periods=N_BATCHES, freq="MS") + pd.Timedelta(days=14)

    titer = rng.normal(5.0, 0.22, N_BATCHES).round(2)
    low = {"XX0007": 3.62, "XX0008": 3.74}  # the triggering excursion
    rows = []
        ...
        rows.append(
            {
                ...
                "product": "mAb-Demo-A",
                "harvest_titer_g_per_l": low.get(bid, float(titer[i - 1])),
                "step_yield_pct": round(float(rng.normal(78, 3)), 1),
                "viable_cell_density_e6": round(float(rng.normal(14.5, 0.8)), 1),
            }
        )
    df = pd.DataFrame(rows)
    # Make the excursion physically consistent: low titer tracks low VCD.
    df.loc[df.batch_id.isin(AFFECTED_BATCHES), "viable_cell_density_e6"] = [11.2, 11.9]
    df.loc[df.batch_id.isin(AFFECTED_BATCHES), "step_yield_pct"] = [69.4, 71.1]
    return df
...
def generate_deviations(seed: int = SEED) -> pd.DataFrame:
    ...
    rows = [
        ...
        ("DEV-2025-023", "2025-03-22", "XX0003", "Measurement (Analytical)",
         "Titer assay (HPLC)", "Minor", "HPLC autosampler error; sample re-injected, result valid."),
...
def generate_change_controls(seed: int = SEED) -> pd.DataFrame:
    ...
    rows = [
        ...
        ("CC-2025-009", "2025-04-02", "Machine (Equipment)",
         ...
         "Protein A chromatography skid firmware update."),
...
def generate_assay_investigations(seed: int = SEED) -> pd.DataFrame:
    ...
    The titer assay investigation *clears* the assay (in control), which should
    push the Measurement category down the priority list — a realistic outcome
    that the evidence-weighting must reflect.
    """
    rows = [
        ("AINV-2025-033", "2025-07-22", "XX0007", "Titer assay (HPLC)",
         "Measurement (Analytical)", "Titer assay (HPLC)", "Cleared",
         "Titer assay re-test and system suitability passed; low titer confirmed as real, not analytical."),
